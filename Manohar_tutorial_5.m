...
% m = 2; 
% while(m <= 12)
%     plot([x_points(m -1),x_points(m)],[y_points(m - 1),y_points(m - 1)],'g');
%     plot([x_points(m),x_points(m)],[y_points(m - 1),y_points(m)],'g');
%     m = m + 1;
% end
...
figure(3)
x_names = {'x (equi)', 'y (equi)','L','V','HL','HV'};
y_names = 1:1:11;
h = heatmap(x_names,y_names,[Xn,Yn,Ln,Vn,HLn,HVn]);

h.Title = 'n-heptane vs ethyl benzene in liquid and vapour phases'; 
h.YLabel = 'Composition of components in each stage';
h.XLabel = 'Components in liquid phase and vapour phase';
